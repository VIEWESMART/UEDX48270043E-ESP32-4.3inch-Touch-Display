<h1 align = "center">VIEWE 4.3" 800*480 ESP32-S3 智能显示屏快速指南 </h1>
...
PLCM-UEDX48270043E-WB-B 是一款基于 ESP32-S3 的高性能智能显示开发板，配备 4.3 英寸 RGB 触摸屏（480x272）。它由 VIEWE 设计，适用于需要丰富外设接口和 Wi-Fi/BLE 连接的物联网及人机界面应用。

> [!NOTE]
> 为区分 **800 x 480** 和 **480 x 272** 套装版本，我们根据显示屏幕的分辨率将本开发板组合命名为**PLCM-UEDX48270043E-WB-A** ,PCB 开发板统一使用**UEDX80480043E-WB-A / B**请注意区分
> UEDX80480043E-WB-A 开发板将**逐步停产**，由 UEDX80480043E-WB-B 开发板替代。
> 建议新设计迁移至此版本。
...
- **存储器**
  ...
  - 16 MB Flash。
- **外设接口**
  - 板载两组 2\*20 排针，引出多个可编程 GPIO，支持 SPI、UART、I2C、I2S、LCD、Camera、USB OTG 等接口。
...
凭借丰富的连接性和强大的处理能力，UEDX80480043E-WB-B 是以下领域物联网设备的理想选择：
...
#### 4.2.1 准备工作
* **硬件**：UPLCM-UEDX48270043E-WB-A 或 PLCM-UEDX48270043E-WB-B 开发板，USB-C 数据线。
...
5.  **配置乐鑫支持的开发板**:
    ...
    * 确保取消注释：`#define BOARD_VIEWE_UEDX48270043E_WB_A`
    ```c
    ...
    /**
...
2.  **配置 PlatformIO**:
    * 此示例默认使用 `BOARD_ESPRESSIF_ESP32_S3_LCD_EV_BOARD_2_V1_5` 开发板。请在 `platformio.ini` 文件的 `[platformio]:default_envs` 中选择 `BOARD_VIEWE_UEDX48270043E_WB_A`。
3.  **配置示例**:
    - [可选] 编辑 `lvgl_v8_port.h` 文件中的宏定义
        - **如果使用 `RGB/MIPI-DSI` 接口**，将 `LVGL_PORT_AVOID_TEARING_MODE` 宏定义修改为 `1`/`2`/`3` 以启用防撕裂功能。之后，将 `LVGL_PORT_ROTATION_DEGREE` 宏定义修改为目标旋转角度。
        - **如果使用其他接口**，请不要修改 `LVGL_PORT_AVOID_TEARING_MODE` 和 `LVGL_PORT_ROTATION_DEGREE` 宏定义。
...
- [产品规格书(WB-A)](datasheet/PLCM-UEDX80480043E-WB-A%20V3.2%20SPEC.pdf)
- [产品规格书(WB-B)](datasheet/PLCM-UEDX80480043E-WB-B%20V1.0%20SPEC.pdf)
- [PCB-UEDX80480043E-WB-A 原理图 (PDF)](Schematic/UEDX48480043E-WB-A%20V1.3.SCH.pdf)
- [PCB-UEDX80480043E-WB-B 原理图 (PDF)](Schematic/UEDX80480043E-WB-B.pdf)
- [2D 图纸 (dwg)](2D%20drawings/PLCM-DX80480043E-WB-B.dwg)
- [显示屏规格书 (PDF)](datasheet/UE043HV-RN40-A058B_V1.1.pdf)
- [显示驱动芯片规格书 (PDF)](datasheet/ILI6485_DTS_V008_20201113.pdf)
